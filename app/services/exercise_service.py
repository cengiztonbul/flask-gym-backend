...
def create_exercise(name, image, video_url, body_parts, desc, application):
    """ application is stored with *s to determine the steps. it will
    ...
    import datetime
    filename = name + "-" + datetime.datetime.now().hour + "-" + datetime.datetime.now().min + "-" + \
                datetime.datetime.now().second + ".png"

    new_exercise.img_url = os.path.join(app.config['UPLOAD_FOLDER'], filename)
...
def get_exercise_by_id(id_str):
    return ExerciseTemplate.objects(id=ObjectId(id)).first()
